...
namespace Lucene.Net.Replicator.Http
{
    ...
    public abstract class HttpClientBase : IDisposable
    {
        ...
        //JAVA: }
        //Note: This goes to the overall timeout
        ...
        //JAVA: }
        //Note: This goes to the Read and Write timeouts in the request (Closest we can get to a socket timeout?)
        ...
        protected void ThrowKnownError(HttpResponseMessage response)
        {
            ...
            {
                //not likely
                throw new Exception(string.Format("Failed to read exception object: {0} {1}", response.StatusCode, response.ReasonPhrase), e);
            }
            finally
            {
                input.Close();
        ...
        protected HttpResponseMessage ExecutePost(string request, object entity, params string[] parameters)
        {
            ...
            EnsureOpen();
            //Note: No headers? No ContentType?... Lucene is not really making good use of Http?
            HttpRequestMessage req = new HttpRequestMessage(HttpMethod.Post, QueryString(request, parameters));
            
            //TODO: Use JsonSerializer + StreamContent, this will do for now.
            req.Content = new StringContent(JToken.FromObject(entity).ToString(Formatting.None), Encoding.UTF8, "application/json");
            //Note: Bridging from Async to Sync, this is not ideal and we could consider changing the interface to be Async or provide Async overloads
