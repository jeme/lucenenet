        ...
        internal class ReaderPool : IDisposable
        {
            ...
            public virtual ReadersAndUpdates Get(SegmentCommitInfo info, bool create)
            {
                lock (this)
                {
                    ...
                    {
                        if (Debugging.AssertsEnabled) Debugging.Assert(rld.Info == info, "rld.info={0} info={1} isLive?={2} vs {3}", rld.Info, info, InfoIsLive(rld.Info), InfoIsLive(info));
                    }
        ...
        private int MergeMiddle(MergePolicy.OneMerge merge)
        {
            ...
            try
            {
                ...
                while (segUpto < sourceSegments.Count)
                {
                    ...
                    if (Debugging.AssertsEnabled) Debugging.Assert(delCount <= info.Info.DocCount, "delCount={0} info.docCount={1} rld.pendingDeleteCount={2} info.getDelCount()={3}", delCount, info.Info.DocCount, rld.PendingDeleteCount, info.DelCount);
                    segUpto++;
                }
