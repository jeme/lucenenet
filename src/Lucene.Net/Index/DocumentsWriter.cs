    ...
    internal sealed class DocumentsWriter : IDisposable
    {
        ...
        private bool DoFlush(DocumentsWriterPerThread flushingDWPT)
        {
            ...
            while (flushingDWPT != null)
            {
                ...
                try
                {
                    if (Debugging.AssertsEnabled) Debugging.Assert(currentFullFlushDelQueue == null || flushingDWPT.deleteQueue == currentFullFlushDelQueue, "expected: {0}but was: {1} {2}", currentFullFlushDelQueue, flushingDWPT.deleteQueue, flushControl.IsFullFlush);
                    /*
