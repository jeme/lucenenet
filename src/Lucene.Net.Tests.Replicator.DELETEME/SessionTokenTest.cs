...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
	  public virtual void testSerialization()
	  {
		...
		SessionToken session2 = new SessionToken(new DataInputStream(new System.IO.MemoryStream(b)));
		...
		IList<RevisionFile> files1 = session1.sourceFiles.values().GetEnumerator().next();
		IList<RevisionFile> files2 = session2.sourceFiles.values().GetEnumerator().next();
